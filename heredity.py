...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    """
    probability = 1
    zero_gene = people.keys() - one_gene
    zero_gene -= two_genes
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        gene_normal_ratio = 1 / sum(probabilities[person]['gene'].values())
        trait_normal_ratio = 1 / sum(probabilities[person]['trait'].values())

        for x in probabilities[person]['gene']:
            probabilities[person]['gene'][x] *= gene_normal_ratio

        for x in probabilities[person]['trait']:
            probabilities[person]['gene'][x] *= trait_normal_ratio
